...
/// A trait for the ability to identify and make Operation (Eq, Ord) on Object with XMI ID
pub trait XMIIdentity: XMIIdentification + PartialEq + Eq + PartialOrd + Ord {}

// ####################################################################################################

/// A trait for the ability to identify on Object with XMI ID
pub trait XMIIdentification {
    /// Return the label (String name) of the XMI identity
    fn get_xmi_label(&self) -> Result<String, anyhow::Error>;
    /// Return the XMI identity
    fn get_xmi_id(&self) -> XMIIdLocalReference;
}

// ####################################################################################################

/// A trait for helping to deserialize a struct using XMIIdentity, XMIIdentification, XMIIdLocalReference or XMIIdReference
pub trait XMIDeserialize {}
...
impl Default for XMIIdLocalReference {
    fn default() -> Self {
        XMIIdLocalReference {
...
#[derive(Clone)]
/// Reference to another XMI object, stored as RefCell
pub struct XMIIdReference<T: Clone> {
    object_id: String,
    package_id: String,
    object: RefCell<Option<T>>, // Content
}
...
impl XMIIdentification for XMIIdLocalReference {
    ...
    fn get_xmi_label(&self) -> anyhow::Result<String, anyhow::Error> {
        self.label()
    }
}
